 ...
 async function auth (req: Request, res: Response, next: any) {
        ...
        if (!_.includes(req.headers.authorization, 'Basic ')) {
            ...
            try {
                ...
                if (isValid) {
                    // Find or create user
                    const [user, created] = await Users.findOrCreate({where: {chain_type: chain.chain_type, address: address }, defaults: {chain_type: chain.chain_type, address: address}})
                ...
                res.status(401).json(Failure.commonErr(error instanceof AuthError ? error.message : 'Invalid Signature'));
                return;
            }
        }
    }
